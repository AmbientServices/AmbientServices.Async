...
namespace AmbientServices
{
    ...
    public sealed class FifoTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        private FifoTaskScheduler(string schedulerName, int bufferThreadCount = 0, int maxThreads = 0, ThreadPriority priority = ThreadPriority.Normal, IAmbientStatistics? statistics = null, int schedulerMasterFrequencyMilliseconds = 1000, bool testMode = false)
        {
            ...
            SchedulerInvocations = _statistics?.GetOrAddStatistic(AmbientStatisticType.Cumulative, $"{nameof(SchedulerInvocations)}:{schedulerName}", $"{nameof(SchedulerInvocations)}:{schedulerName}", "The total number of scheduler invocations");
            SchedulerInvocationTime = _statistics?.GetOrAddTimeBasedStatistic(AmbientStatisticType.Cumulative, $"{nameof(SchedulerInvocationTime)}:{schedulerName}", $"{nameof(SchedulerInvocationTime)}:{schedulerName}", "The total number of ticks spent doing invocations");
            SchedulerWorkersCreated = _statistics?.GetOrAddStatistic(AmbientStatisticType.Cumulative, $"{nameof(SchedulerWorkersCreated)}:{schedulerName}", $"{nameof(SchedulerWorkersCreated)}:{schedulerName}", "The total number of scheduler workers created");
            SchedulerWorkersRetired = _statistics?.GetOrAddStatistic(AmbientStatisticType.Cumulative, $"{nameof(SchedulerWorkersRetired)}:{schedulerName}", $"{nameof(SchedulerWorkersRetired)}:{schedulerName}", "The total number of scheduler workers retired");
            SchedulerInlineExecutions = _statistics?.GetOrAddStatistic(AmbientStatisticType.Cumulative, $"{nameof(SchedulerInlineExecutions)}:{schedulerName}", $"{nameof(SchedulerInlineExecutions)}:{schedulerName}", "The total number of scheduler inline executions");
            SchedulerWorkers = _statistics?.GetOrAddStatistic(AmbientStatisticType.Raw, $"{nameof(SchedulerWorkers)}:{schedulerName}", $"{nameof(SchedulerWorkers)}:{schedulerName}", "The current number of scheduler workers");
            SchedulerBusyWorkers = _statistics?.GetOrAddStatistic(AmbientStatisticType.Raw, $"{nameof(SchedulerBusyWorkers)}:{schedulerName}", $"{nameof(SchedulerBusyWorkers)}:{schedulerName}", "The current number of busy scheduler workers");
            SchedulerWorkersHighWaterMark = _statistics?.GetOrAddStatistic(AmbientStatisticType.Max, $"{nameof(SchedulerWorkersHighWaterMark)}:{schedulerName}", $"{nameof(SchedulerWorkersHighWaterMark)}:{schedulerName}", "The highest number of scheduler workers");
            SchedulerSlowestInvocationMilliseconds = _statistics?.GetOrAddStatistic(AmbientStatisticType.Max, $"{nameof(SchedulerSlowestInvocationMilliseconds)}:{schedulerName}", $"{nameof(SchedulerSlowestInvocationMilliseconds)}:{schedulerName}", "The highest number of milliseconds required for an invocation");
